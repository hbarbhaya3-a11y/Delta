...
export const SIGNAL_HUB_CLOSURE = {
  ...
  description: 'Forecast-driven signal translating weather risk into operational exposure. Converts raw weather inputs into network impact—which flights, banks, and crew rotations will be affected if a hub shuts down. Earliest trigger for pre-disruption simulation. Scenario: ATL storms + ATC ground delay, T-30h→T-6h.',
  metrics: [
    { label: 'Closure Likelihood', value: '72%', description: '6–10 hr disruption window' },
    { label: 'Exposed Flights', value: '184', description: 'Flights in the closure window' },
    { label: 'Exposed Pairings', value: '52', description: 'Rotations starting/connecting via ATL' },
    { label: 'Forecast Confidence', value: '88%', description: 'Weather forecast certainty' },
  ],
  response_window: 'T-30h → T-6h',
  likely_root_causes: [
    'Thunderstorm severity (runway/gate disruption)',
    'ATC ground delay program (-38% departures)',
    'Airport capacity constraints',
  ],
  recommended_actions: [
    'Pre-cancel low-value flights',
    'Pre-position + activate reserves',
    'Protect high-value banks',
    'Alternate hub routing (DTW/MSP)',
  ],
  ready_scenario: 'ATL Hub Closure — Weather Window',
}
...
export const SIGNAL_RESERVE_BURN = {
  ...
  severity: '🟠 High',
  signal_class: 'Capacity depletion / Level 0→1 early warning',
  description: 'Multi-hub reserve-depletion event across ATL/JFK/DTW/MSP from ATL weather, an open-trip inflow spike, and crew sick calls. Reserves burn faster than open trips close; escalates to Critical when coverage falls below threshold and swaps create donor-flight risk. Scenario: L0 active, L1 partially active, L2 watch.',
  metrics: [
    { label: 'Network Burn Rate', value: '11.8 / hr', description: 'Reserve crew-equivalents consumed per hour' },
    { label: 'Reserve Coverage', value: '0.72x', description: 'Feasible reserve supply vs 6h demand (0.47x at ATL)' },
    { label: 'Net Open-Trip Growth', value: '+3.3 / hr', description: 'Inflow 8.4/hr minus closure 5.1/hr' },
    { label: 'Level 1 Probability', value: '57%', description: 'Zero-sum crew-pull risk' },
  ],
  response_window: 'First ATL exhaustion threshold in 3h 10m',
  likely_root_causes: [
    'Crew sick calls + reserve depletion',
    'ATL weather / ATC compression',
    'Open-trip inflow rising faster than closure',
    'Donor-hub coupling (DTW/MSP)',
  ],
  recommended_actions: [
    'Rebalance donor reserves under a 0.95x coverage guardrail',
    'Thin low-criticality flights to cut reserve demand',
    'Protect ATL/JFK banks; block harmful zero-sum swaps',
    'Arm the 24–72h next-day restart watch',
  ],
  ready_scenario: 'Multi-Hub Reserve Burn Containment — ATL/JFK/DTW/MSP',
}
...
export const SIGNAL_CROSS_HUB_PROPAGATION = {
  ...
  description: 'Measures risk of disruption spreading beyond the original hub. Level 2 cascade detection. Scenario: ORD → ATL → JFK → BOS, Level 1 active with Level 2 rising — donor pulls contaminating other hubs and next-day operations.',
  metrics: [
    { label: 'Propagation Probability', value: '68%', description: 'Risk of spread to other hubs' },
    { label: 'Affected Hubs', value: '4', description: 'ORD, ATL, JFK, BOS' },
    { label: 'New Open Trips (non-source)', value: '23', description: 'Confirms Level 1 spread beyond ORD' },
    { label: 'Confidence', value: '89%', description: 'Cascade model confidence' },
  ],
  response_window: '6–24h containment · 24–72h restart',
  likely_root_causes: [
    'Zero-sum crew shifts (donor pulls)',
    'Reserve depletion (ATL 0.69x / JFK 0.81x)',
    'Mispositioned crew + tail/crew splits',
  ],
  recommended_actions: [
    'Block harmful crew pulls',
    'Contain source hub',
    'Protect next-day launch (24–72h restart)',
  ],
  ready_scenario: 'ORD → ATL → JFK → BOS Cross-Hub Contamination Run',
}
...
export const SIGNAL_MISCONNECT_EXPOSURE = {
  ...
  severity: '🟠 High',
  signal_class: 'Passenger-impact / connection cascade',
  description: 'Multi-hub connection cascade ATL → JFK → BOS. ATL bank compression spills into the JFK late bank and BOS next-day wave. Proactive pre-bank optimization + real-time connection protection with passenger segmentation and a balanced CX + network bias. Scenario: L0 active ATL, L1 watch JFK, L2 watch BOS.',
  metrics: [
    { label: 'Misconnects at Risk', value: '1,460', description: 'Passengers projected to miss onward flights' },
    { label: 'Critical Connections', value: '386', description: 'Tight / international / premium / low-reaccom journeys' },
    { label: 'Premium / International', value: '214 / 132', description: 'High-value + harder-to-reaccommodate exposure' },
    { label: 'Confidence', value: '87%', description: 'Misconnection probability' },
  ],
  response_window: 'Proactive T-3h to bank · real-time T-45m to T+20m',
  likely_root_causes: [
    'ATL bank compression (inbound + crew-reserve delay)',
    'Gate congestion + transfer-time friction',
    'Aircraft tail readiness delays',
    'ATC flow constraint into JFK',
  ],
  recommended_actions: [
    'Hold only high-leverage connection-critical flights',
    'Protect premium / international / tight / low-seat segments first',
    'Reaccommodate before the JFK bank closes',
    'Arm the BOS next-day backlog watch',
  ],
  ready_scenario: 'ATL → JFK → BOS Connection Protection Run',
}
...
export const SIGNAL_TAIL_CREW_SYNC = {
  ...
  signal_class: 'Cross-domain constraint / crew-aircraft sync',
  description: 'Multi-hub aircraft/crew readiness mismatch ATL → JFK → BOS (MCO donor). Crew ready without a released tail, or a ready tail without positioned crew. Prevents solving crew without aircraft feasibility — a tail-only or crew-only fix still fails. Scenario: L0 active, L1 watch, L2 next-day watch.',
  metrics: [
    { label: 'Crew Ready vs Tail Ready Gap', value: '+68 min', description: 'Crew ready, assigned tail not ready' },
    { label: 'Affected Legs', value: '18', description: 'Legs with direct or downstream sync gaps' },
    { label: 'Aircraft/Crew Split Pairs', value: '9', description: 'Core signal objects (7 tails · 11 pairings)' },
    { label: 'Feasible Tail Swaps', value: '4', description: 'Ready tails, no maintenance conflict' },
  ],
  response_window: 'T-4h to first departure · live refresh every 10 min',
  likely_root_causes: [
    'Tail release delay + maintenance readiness gap',
    'Crew reassignment / reserve movement mismatch',
    'ATL/JFK bank dependency amplification',
    'Gate congestion extending tail turn time',
  ],
  recommended_actions: [
    'Swap ready tails while preserving legal crews',
    'Resequence downstream rotations',
    'Reassign crew or controlled-delay for maintenance release',
    'Activate BOS 24–72h restart watch',
  ],
  ready_scenario: 'ATL Tail-Crew Synchronization Run — JFK/BOS Spillover',
}
...
export const SIGNAL_POLICY_CONSTRAINT = {
  ...
  description: 'Flags when recommended recovery actions violate legality, contract, or policy constraints. Hard feasibility gate — prevents invalid decisions from entering execution. Scenario: ATL multi-option recovery, 23 of 64 options removed before ranking.',
  metrics: [
    { label: 'Binding Constraints', value: '16', description: 'Constraints blocking/narrowing recovery' },
    { label: 'Infeasible Options', value: '23', description: 'Options removed before ranking (of 64)' },
    { label: 'Exception Required', value: 'Yes (2)', description: 'Conditional options needing approval' },
    { label: 'Approval Tier', value: 'Tier 2', description: 'OCC + Policy leadership' },
  ],
  response_window: 'Before decision approval',
  likely_root_causes: [
    'Duty/rest violations (7)',
    'Qualification / pairing mismatch (4)',
    'Aircraft / maintenance blocks (5)',
    'Policy / contract + cost-cap breaches',
  ],
  recommended_actions: [
    'Eliminate infeasible options',
    'Rank compliant alternatives first',
    'Escalate exceptions with evidence pack',
  ],
  ready_scenario: 'ATL Policy Feasibility Gate — JFK/BOS Spillover',
}
...
export const NRS_LIVE_SIGNALS = [
  ...
  },
  {
    ...
    linkedUseCaseId: 'uc-hub-closure',
    severity: 'CRITICAL', severityColor: 'red', stage: 'SENSE',
    title: 'Hub Closure Likelihood — Weather Window Emerging',
    description: 'Forecast-driven signal translating weather risk into operational exposure.',
    detail: 'Converts raw weather inputs into network impact — which flights, banks, and crew rotations will be affected if a hub shuts down. Earliest trigger for pre-disruption simulation. Scenario: severe storms + active ATC ground delay program at ATL, T-30h→T-6h, -38% departure throughput. Likely root causes: thunderstorm severity, ATC ground delay program, airport capacity constraints.',
    sourceChip: 'WEATHER API · ATC · AIRPORT OPS',
    agent: 'Hub Weather Sentinel', date: '7/13/2026',
    metricValue: '72%', metricUnit: 'closure likelihood',
    metricStripLabel: 'Closure likelihood', metricSub: '184 exposed flights · 52 pairings',
    tags: ['CLOSURE 72%', 'EXPOSED BANKS', '52 PAIRINGS', 'ATC GROUND DELAY', 'ATL EVENING BANK'],
    precedents: 3, precedentNote: null, confidence: 88,
    window: 'T-30h → T-6h',
    trendLabel: 'HUB CLOSURE LIKELIHOOD TREND — Forecast Window',
    trendData: [
      { x: 'T-30', v: 31 }, { x: 'T-24', v: 44 }, { x: 'T-18', v: 55 },
      { x: 'T-12', v: 64 }, { x: 'Now', v: 72 },
    ],
    scenario: 'ATL Hub Closure — Weather Window',
    scenarioSub: 'Weather forecast → hub/bank exposure → pre-cancel + reserve pre-position (contain at L0)',
  },
  {
    ucId: 'nrs-uncovered-trip',
    linkedUseCaseId: 'uc-uncovered-trip',
    severity: 'CRITICAL', severityColor: 'red', stage: 'SENSE',
    title: 'Uncovered Trip Detected — Time-to-Departure Critical',
    description: 'Flags flights without assigned crew within the critical execution window.',
    detail: 'Triggered when trips become uncovered after sick calls. With ~2% voluntary acceptance, waiting for pickup is ineffective — this signal forces immediate simulation instead of reactive waiting. Scenario: 9 concurrent open trips at ATL (6 inside T-5h) coupled to JFK/LGA/MCO/DTW, coverage 46% no action → 78% with plan, L0 active → L1 trigger risk in 90–120 min. Likely root causes: crew sick call/no-show, low acceptance, legality breach, mispositioned crew.',
    sourceChip: 'CREW SCHEDULING · LEGALITY FEED',
    agent: 'Crew Coverage Sentinel', date: '7/13/2026',
    metricValue: '9', metricUnit: 'open trips',
    metricStripLabel: 'Open trips', metricSub: '6 inside T-5h · acceptance ~2%',
    tags: ['9 OPEN TRIPS', 'T-5h · T-2h LOCK', 'COVERAGE 46%→78%', 'ACCEPTANCE ~2%', 'L0/L1 CASCADE'],
    precedents: 7, precedentNote: null, confidence: 92,
    window: 'T-5h · T-2h execution lock',
    trendLabel: 'UNCOVERED TRIP TREND — Open Trips in Execution Window',
    trendData: [
      { x: 'T-8h', v: 2 }, { x: 'T-7h', v: 4 }, { x: 'T-6h', v: 6 },
      { x: 'T-5h', v: 7 }, { x: 'T-4h', v: 8 }, { x: 'T-3h', v: 9 }, { x: 'Now', v: 9 },
    ],
    scenario: 'ATL Multi-Open Trip Recovery — T-5h',
    scenarioSub: 'Uncovered trip → reserve activation / crew swap / incentive → delay or cancel decision',
  },
  {
    ucId: 'nrs-reserve-burn',
    linkedUseCaseId: 'uc-reserve-burn',
    severity: 'HIGH', severityColor: 'orange', stage: 'SENSE',
    title: 'Reserve Burn Rate Acceleration',
    description: 'Reserves burning faster than open trips close across ATL/JFK/DTW/MSP.',
    detail: 'Multi-hub reserve-depletion event driven by ATL weather, an open-trip inflow spike (8.4/hr vs 5.1/hr closure), and crew sick calls. Coverage 0.72x network-wide (0.47x ATL) with the earliest exhaustion ETA at ATL (3h 10m). Level 0 prevention + Level 1 zero-sum containment under a donor guardrail. Scenario: L0 active, L1 partially active, L2 watch. Likely root causes: sick calls + reserve depletion, ATL weather/ATC, open-trip inflow, donor-hub coupling.',
    sourceChip: 'CREW · NETWORK · AIRCRAFT · PAX · WX/ATC',
    agent: 'Reserve Sentinel', date: '7/13/2026',
    metricValue: '0.72x', metricUnit: 'reserve coverage',
    metricStripLabel: 'Reserve coverage', metricSub: 'burn 11.8/hr · ATL exhaustion 3h 10m',
    tags: ['BURN 11.8/hr', 'COVERAGE 0.72x', 'NET GROWTH +3.3/hr', 'LEVEL 1 57%', 'ATL/JFK/DTW/MSP'],
    precedents: 3, precedentNote: null, confidence: 86,
    window: 'First ATL exhaustion threshold in 3h 10m',
    trendLabel: 'RESERVE BURN TREND — Network Coverage Ratio (x100)',
    trendData: [
      { x: 'T-8', v: 118 }, { x: 'T-6', v: 102 }, { x: 'T-4', v: 88 },
      { x: 'T-2', v: 76 }, { x: 'Now', v: 72 },
    ],
    scenario: 'Multi-Hub Reserve Burn Containment — ATL/JFK/DTW/MSP',
    scenarioSub: 'Burn acceleration → rebalance under donor guardrail + thin demand / protect banks / prevent Level 1',
  },
  {
  ...
  },
  {
    ...
    linkedUseCaseId: 'uc-cross-hub-propagation',
    severity: 'CRITICAL', severityColor: 'red', stage: 'SENSE',
    title: 'Cross-Hub Propagation Probability',
    description: 'Measures risk of disruption spreading beyond the original hub.',
    detail: 'Level 2 cascade detection. Scenario: ORD → ATL → JFK → BOS — already in Level 1 zero-sum crew swaps with Level 2 probability rising. Donor pulls are creating non-source open trips and threatening the BOS next-day first bank. Likely root causes: zero-sum crew shifts, reserve depletion, mispositioned crew, tail/crew splits.',
    sourceChip: 'MULTI-HUB · CREW · AIRCRAFT · NEXT-DAY OPS',
    agent: 'Cascade Sentinel', date: '7/13/2026',
    metricValue: '68%', metricUnit: 'propagation prob',
    metricStripLabel: 'Propagation probability', metricSub: '4 hubs · 23 non-source open trips',
    tags: ['PROPAGATION 68%', 'AFFECTED HUBS 4', 'DONOR-FLIGHT RISK', 'NEXT-DAY CONTAMINATION', 'LEVEL 2 WATCH'],
    precedents: 3, precedentNote: null, confidence: 89,
    window: '6–24h containment · 24–72h restart',
    trendLabel: 'CROSS-HUB PROPAGATION TREND — Contamination Probability',
    trendData: [
      { x: 'T-6h', v: 22 }, { x: 'T-4h', v: 34 }, { x: 'T-3h', v: 46 },
      { x: 'T-2h', v: 55 }, { x: 'T-1h', v: 62 }, { x: 'Now', v: 68 },
    ],
    scenario: 'ORD → ATL → JFK → BOS Cross-Hub Contamination Run',
    scenarioSub: 'Detect spread → block donor swaps → contain at source → protect next-day launch',
  },
  {
    ucId: 'nrs-misconnect-exposure',
    linkedUseCaseId: 'uc-misconnect-exposure',
    severity: 'HIGH', severityColor: 'orange', stage: 'SENSE',
    title: 'Passenger Misconnect Exposure',
    description: 'ATL bank compression spilling into the JFK late bank and BOS next-day wave.',
    detail: 'Multi-hub connection cascade ATL → JFK → BOS. Inbound + crew-reserve delays, gate congestion, and an ATC flow constraint compress the ATL 17:30–20:30 bank; late arrivals miss the JFK outbound bank and threaten the BOS next-day first wave. Balanced CX + network bias with passenger segmentation. Scenario: L0 active ATL, L1 watch JFK, L2 watch BOS. Likely root causes: bank compression, crew swaps/reserve delay, aircraft readiness, gate congestion, ATC.',
    sourceChip: 'PASSENGER · NETWORK · CREW · AIRCRAFT · GATE · ATC',
    agent: 'Connection Sentinel', date: '7/13/2026',
    metricValue: '1,460', metricUnit: 'misconnects at risk',
    metricStripLabel: 'Misconnects at risk', metricSub: '386 critical · 214 premium · 132 intl',
    tags: ['MISCONNECTS 1,460', 'CRITICAL 386', 'PREMIUM + INTL', 'ATL/JFK/BOS BANKS', 'L0/L1/L2 CASCADE'],
    precedents: 3, precedentNote: null, confidence: 87,
    window: 'Proactive T-3h · real-time T-45m to T+20m',
    trendLabel: 'MISCONNECT EXPOSURE TREND — Passengers at Risk',
    trendData: [
      { x: 'T-6', v: 210 }, { x: 'T-5', v: 470 }, { x: 'T-4', v: 720 },
      { x: 'T-3', v: 980 }, { x: 'T-2', v: 1180 }, { x: 'T-1', v: 1340 }, { x: 'Now', v: 1460 },
    ],
    scenario: 'ATL → JFK → BOS Connection Protection Run',
    scenarioSub: 'Bank compression → hold high-leverage flights / protect premium+intl / reaccommodate / prevent JFK+BOS spillover',
  },
  {
    ucId: 'nrs-tail-crew-sync',
    linkedUseCaseId: 'uc-tail-crew-sync',
    severity: 'MEDIUM-HIGH', severityColor: 'yellow', stage: 'SENSE',
    title: 'Tail-Crew Synchronization Gap',
    description: 'Crew and aircraft readiness out of phase across ATL → JFK → BOS.',
    detail: 'Multi-hub aircraft/crew mismatch ATL → JFK → BOS (MCO donor). Crew ready without a released tail (DL2381 ATL–JFK) and a ready tail without positioned crew (DL1842 ATL–MCO) — 9 split pairs. A tail-only or crew-only fix still fails; recovery must synchronize both sides with maintenance, gate, and next-day restart feasibility. Scenario: L0 active, L1 watch, L2 next-day watch. Likely root causes: tail release delay, maintenance readiness, crew reassignment, gate congestion.',
    sourceChip: 'AIRCRAFT · CREW · NETWORK · PAX · MAINTENANCE',
    agent: 'Sync Sentinel', date: '7/13/2026',
    metricValue: '68', metricUnit: 'min readiness gap',
    metricStripLabel: 'Readiness gap', metricSub: '18 legs · 9 split pairs · 7 tails',
    tags: ['READINESS GAP 68m', 'SPLIT PAIRS 9', 'TAIL SWAP OPTIONS 4', 'MAINTENANCE GATES 3', 'L0/L1/L2 CASCADE'],
    precedents: 3, precedentNote: null, confidence: 81,
    window: 'T-4h to first departure · live refresh 10 min',
    trendLabel: 'TAIL-CREW SYNC TREND — Readiness Gap (min)',
    trendData: [
      { x: 'T-6', v: 12 }, { x: 'T-5', v: 24 }, { x: 'T-4', v: 38 },
      { x: 'T-3', v: 49 }, { x: 'T-2', v: 58 }, { x: 'T-1', v: 63 }, { x: 'Now', v: 68 },
    ],
    scenario: 'ATL Tail-Crew Synchronization Run — JFK/BOS Spillover',
    scenarioSub: 'Crew↔tail mismatch → swap ready tails + preserve legal crews / resequence / protect BOS restart',
  },
  {
    ucId: 'nrs-policy-constraint',
    linkedUseCaseId: 'uc-policy-feasibility-gate',
    severity: 'CRITICAL', severityColor: 'red', stage: 'SENSE',
    title: 'Binding Policy / Contract Constraint Flag',
    description: 'Flags when recommended recovery actions violate legality, contract, or policy constraints.',
    detail: 'Hard feasibility gate. Scenario: ATL multi-option recovery (JFK/BOS/MCO downstream) — 16 binding constraints removed 23 of 64 options before ranking; 2 options need a Tier 2 exception. Directly tied to the recommendation engine\'s Stage 1 filtering. Likely root causes: duty/rest violations, qualification mismatch, maintenance blocks, policy/contract constraints.',
    sourceChip: 'POLICY · LEGALITY · AIRCRAFT · COST',
    agent: 'Policy Sentinel', date: '7/13/2026',
    metricValue: '16', metricUnit: 'binding constraints',
    metricStripLabel: 'Binding constraints', metricSub: '23 of 64 options removed · Tier 2',
    tags: ['16 BINDING CONSTRAINTS', '23 INFEASIBLE OPTIONS', 'EXCEPTION REQUIRED', 'TIER 2 APPROVAL', 'FEASIBILITY GATE'],
    precedents: 3, precedentNote: null, confidence: 89,
    window: 'Before decision approval · T-2h 25m',
    trendLabel: 'POLICY CONSTRAINT TREND — Binding Constraints Flagged',
    trendData: [
      { x: 'T-6h', v: 4 }, { x: 'T-5h', v: 7 }, { x: 'T-4h', v: 10 },
      { x: 'T-3h', v: 12 }, { x: 'T-2h', v: 14 }, { x: 'Now', v: 16 },
    ],
    scenario: 'ATL Policy Feasibility Gate — JFK/BOS Spillover',
    scenarioSub: 'Filter infeasible options → rank compliant / high-cost / exception → auditable release',
  },
]
